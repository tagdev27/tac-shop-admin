...
export class SettingsComponent implements OnInit {
  ...
  getRoles() {
    firebase.firestore().collection('db').doc('tacadmin').collection('roles').onSnapshot(query => {
      this.data = []
      this.roles = []
      var index = 0
      query.forEach(data => {
  ...
  getUsers() {
    firebase.firestore().collection('db').doc('tacadmin').collection('users').onSnapshot(query => {
      this.data2 = []
      var index = 0
      query.forEach(data => {
  ...
  levels = [
    ...
    { value: 'Store Settings', viewValue: 'Store Settings' },
  ...
  deleteUser(user:any) {
    ...
    swal({
      ...
      if (result.value) {
        firebase.firestore().collection('db').doc('tacadmin').collection('users').doc(id).delete().then(del => {
  ...
  deleteRole(id: string, role: string) {
    ...
    swal({
      ...
      if (result.value) {
        firebase.firestore().collection('db').doc('tacadmin').collection('roles').doc(id).delete().then(del => {
          this.config.displayMessage("Successfully deleted", true);
        }).catch(err => {
          this.config.displayMessage(`${err}`, false);
        })
  ...
  roleButtonAction() {
    ...
    if (this.isAddRole) {
      ...
      firebase.firestore().collection('db').doc('tacadmin').collection('roles').doc(key).set(rolePush).then(d => {
        this.modalService.dismissAll()
      ...
      firebase.firestore().collection('db').doc('tacadmin').collection('roles').doc(key).update(rolePush).then(d => {
        this.modalService.dismissAll()
        this.button_pressed = false
        this.isAddRole = true
        this.currentLevel = []
